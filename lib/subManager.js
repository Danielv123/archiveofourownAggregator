...
console.log("Loaded database: "+settings.databaseName);
...
function checkForUpdates(){
	...
	function updateChecker(data){
		let path = urlparser.parse(userdata.storyURL).path.base.split("/");
	...
	for(let i = 0; i < database.archiveofourown.length; i++){
		setTimeout(()=>updateChecker(database.archiveofourown[i]),i*3000);
	}
}
...
function handleNewChapterFound(data){
	console.log("New chapter found in: "+data.title);
